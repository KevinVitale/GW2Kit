    ...
    [GW2 eventStatesWithParameters:@{@"world_id" : @"1005", @"map_id" : @"62"}
                        completion:^(NSError *error, id states) {
                            for(GW2EventStatus *status in states) {
                                GW2ResourceName *resourceName = [eventNames objectAtIndex:[[eventNames valueForKey:@"id"] indexOfObject:[status eventID]]];
                                printf("%s", [status description].UTF8String);
                                printf("  %s\n", resourceName.name.UTF8String);
                            }
...
- (void)testMaps_Synchronously {
    [GW2 namesForResource:@"map"
               parameters:nil
               completion:^(NSError *error, NSArray *maps) {
                   for(GW2ResourceName *map in maps) {
...
- (void)testWorlds_Synchronously {
    [GW2 namesForResource:@"world"
               parameters:nil
               completion:^(NSError *error, NSArray *worlds) {
                   for(GW2ResourceName *world in worlds) {
...
- (void)testWvWObjects_Synchronously {
    [GW2 namesForResource:@"wvw/objective"
               parameters:nil
               completion:^(NSError *error, NSArray *objectives) {
                   for(GW2ResourceName *objective in objectives) {
